...
const consumers = {
    registry: registry,
    register: (name, options, onStateUpdate) => {
        ...
        const consumer = new (class extends Consumer {
            ...
            constructor(...args) {
                try {
                    ...
                    console.log(err)
                    throw (`
Illegal constructor instructions for ${ns} API listener.
[send/on/once] should\'nt be called at invocation.
Because of the internal automated API crossing dependency system it's recommanded to use hook effects or mounting methods.
Try something like this on the main component that requires crossing API features: 

    React.useEffect( () => 
    { 
        API.yourapi.connected && 
            API.yourapi.once('receive_data') &&
            API.yourapi.send('ask_for_data')
        
        API.anotherapi.on('logged_off', API.yourapi.close())
    }, [ 
        API.anotherapi.authenticated && API.yourapi.connected
    ]) 
`)
                }
            }
            ...
            connect() {
                ...
                this.socket.addEventListener('message', e => this.receive());
                this.updateState()
            }
